    ...
    for cluster in data:
        ...
        for question, claims_data in cluster["questions"].items():
            ...
            for claim_id, claim in enumerate(claims_data["claims"]):
                ...
                md_text += '|<div style="max-width: 200px;"><span class="anchor" id="{}"></span><font  color=#FF3399>[{}]</font> {}</div>|<div style="display: flex; justify-content: center; max-width: 150px; align-items: center; flex-direction: column;"><a href="{}" target="_blank"><ClientOnly><BiasChart bias="{}" /></ClientOnly></a><div><a href="{}" target="_blank">{}</a></div><div>{}</div><div>{}</div></div>|{}|\n'.format(
                    claim_id + 1,
                    claim_id + 1,
                    claim["sentence"],
                    source["allsides_url"],
